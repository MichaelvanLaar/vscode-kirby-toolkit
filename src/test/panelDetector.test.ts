...
suite("Panel Detector Test Suite", () => {
  suite("isServerRunning", () => {
    test("should return false for non-existent server", async function () {
      this.timeout(3000); // Allow room for the internal 2s fetch timeout
    ...
    test("should not hang past the internal 2s timeout", async function () {
      this.timeout(3000); // Allow 3 seconds for test

      const startTime = Date.now();
      // Use a non-routable IP; some environments reject it immediately,
      // others hang until the internal AbortController fires at 2s.
      const result = await isServerRunning("http://192.0.2.1:8000/panel");
      const duration = Date.now() - startTime;

      assert.strictEqual(result, false);
      // Must never exceed the internal timeout by more than a small margin
      assert.ok(
        duration < 2500,
        `Expected resolution within 2500ms, got ${duration}ms`,
      );
    });
  });
  ...
  suite("Port extraction", () => {
    ...
    test("should handle commands without ports", () => {
      const commands = ["npm run dev", "php -S localhost", "serve"];
